...
{
  var displayname = "How To Start a Startup by Sam Altman - Part 1"; 
  var tagline = "Nuggets from the first 7 lectures"; 
...
function runQuery()
{
  ...
  query.greaterThan("createdAt", new Date(2014, 8, 1))
  query.lessThan("createdAt", new Date(2014, 11, 1))
    ...
    success: function(results_nugget_user) {
      ...
      else
      {
        var results = $.map(results_nugget_user, function(nugget_user) {
          ...
          if(($.inArray('HowToStartAStartup', return_object.tags) > -1)||($.inArray('howToStartAStartup', return_object.tags) > -1))
